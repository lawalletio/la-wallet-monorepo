    ...
    <Combobox $isSelected={select !== 0}>
      <Trigger onClick={() => setOpen(!open)} className="trigger">
        <Flex align="center" flex={1} gap={8}>
          <Flex flex={1}>
            {select ? <TokenInfo item={list.find((item) => item.id === select)} showPrice={showPrice} /> : children}
          </Flex>
          <Icon size="small">
            <FontAwesomeIcon icon={faSort} color={open ? theme.colors.text : theme.colors.gray30} />
          </Icon>
        </Flex>
      </Trigger>
      {list && list.length > 0 && (
        <Content $isOpen={open}>
          <Group>
            {list?.map((item) => (
              <Item key={item.id} tabIndex={open ? 0 : -1} onClick={() => handleClick(item.id)}>
                <TokenInfo item={item} />
                {select === item.id && <FontAwesomeIcon icon={faCheck} color={theme.colors.primary} />}
              </Item>
            ))}
          </Group>
        </Content>

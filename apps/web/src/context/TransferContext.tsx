...
export function TransferProvider({ children, tokenName }: { children: React.ReactNode; tokenName: string }) {
  const transferValue = useTransfer({ tokenName });

  return <TransferContext.Provider value={transferValue}>{children}</TransferContext.Provider>;
}
